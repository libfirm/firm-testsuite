...
FIRMVERSION="1.21.0"
grab "http://switch.dl.sourceforge.net/sourceforge/libfirm/libfirm-$FIRMVERSION.tar.bz2" "TODO:MD5"
CPARSERVERSION="0.9.14"
grab "http://switch.dl.sourceforge.net/sourceforge/cparser/cparser-$CPARSERVERSION.tar.bz2" "TODO:MD5"
cd "$BUILDDIR"
echo "Extracting..."
tar -xf "$PACKAGEDIR/libfirm-$FIRMVERSION.tar.bz2"
tar -xf "$PACKAGEDIR/cparser-$CPARSERVERSION.tar.bz2"
cd "libfirm-$FIRMVERSION"
patch -p1 < "$TOPDIR/libfirm-$FIRMVERSION.patch"
...
sed -e 's/-O0/-O2/g' -i Makefile # cparser fails on -O0 :-(
